...
iPXE-Stateless 是一个面向无状态（Stateless）云原生计算环境的**网络引导固件构建仓库**：为无状态算力提供高质量、可复现、许可证边界清晰的统一引导固件，并作为 iPXE-All-Ready 平台的固件底座。仓库不包含 iPXE 源码，仅维护差异补丁与构建资产，可在任意上游基线之上重建。

**适用范围**：聚焦 **2.5G / 5G / 10G 及以上速率高性能网卡**的驱动移植与适配，当前覆盖 RTL8125（2.5G）与 RTL8126（5G）系列；普通千兆 / 百兆网卡仅维持上游基线既有支持，不进行新的适配与更新。

## 关联项目

- **[iPXE-All-Ready](https://github.com/dutyc/ipxe-all-ready)** —— 主仓库：把「无状态」贯彻到**算力层本身**的云原生实现——算力不绑定任何具体硬件，节点插电即活、可丢弃、可瞬间重建。

本仓库与主仓库是同一理念的一体两面：**主仓库让算力无状态，本仓库让引导固件无状态**——固件在此构建，由主仓库消费。
