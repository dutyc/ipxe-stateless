# 固件能力实现参考（iPXE-All-Ready 集成用）

> 本文档仅中文。服务对象：主仓库 iPXE-All-Ready（算力层无状态运转）的开发者与集成者。
> 本仓库 ipxe-stateless 是 iPXE-All-Ready 引导链最底层的**固件底座**：两仓库语义一致、职责严格分离（固件构建 vs 算力无状态运转）。
...
| 仓库 | 职责 | 交付物 |
|---|---|---|
| **iPXE-All-Ready**（主仓库） | 算力层无状态运转：控制面、动态变量链、身份注入、节点注册、盘机解耦、NBFT 消费 | 控制面服务、引导链脚本（boot.ipxe 等）、initramfs 集成 |
| **ipxe-stateless**（本仓库，固件底座） | 固件构建与定制：协议驱动、认证、设备信息采集、网卡适配 | `dist/` 固件产物、`patches/` 补丁链、`test/` 测试脚本 |
