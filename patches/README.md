...
- 上游仓库：`https://github.com/ipxe/ipxe.git`
- 基线提交：`e6e51ccbf17ff40a899c8859fb4e95abd5cfcd57`（master）
- 生成方式（参照旧工作区）：

  ```bash
  git -C <旧工作区/ipxe> diff origin/master -- <文件...> > patches/NNNN-xxx.patch
  ```
...
| 补丁 | 修改文件 | 内容 |
|---|---|---|
| `0001-realtek-8125-adaptation.patch` | `src/drivers/net/realtek.c`、`realtek.h` | RTL8125 全系适配（XID 0x688 版本表、EPHY 初始化表、32 位中断寄存器、FETCH/PAUSE_SLOT、BAR 0x4808、TPPOLL_8125）；RTL8168 改为不认领（由固件 SNP 驱动兜底，规避 native 初始化在部分平台挂起） |
...
2. 用旧工作区（或上次构建缓存 `.cache/ipxe-upstream`）**在新基线上重新生成补丁**：
...
4. 运行 `./build/build.sh` 验证补丁可干净应用、产物功能正常（重点回归：8125 引导、8168 走 SNP 不挂起）。
